...
final class ReactionContextBackgroundNode: ASDisplayNode {
    ...
    func update(
        ...
        if let glassBackgroundView = self.glassBackgroundView {
            ...
            glassBackgroundView.update(size: glassBackgroundFrame.size, cornerRadius: 23.0, isDark: true, tintColor: .init(kind: .panel, color: UIColor(rgb: 0x4d4f5c, alpha: 0.6)), transition: ComponentTransition(transition))
